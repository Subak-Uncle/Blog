![image](https://github.com/Subak-Uncle/Subak-Uncle/assets/115992753/2757fec5-a9c6-457a-8b26-116adb60a40f)
...
1. 무상태성 (Stateless) => 위배한 것이 "session"
    - REST는 HTTP의 특성을 이용하기 때문에 `무상태성`을 가진다.
    - 서버에서는 상태정보를 기억할 필요가 없고 들어온 요청에 대해 처리만 해주면 되기 때문에 **구현이 단순**해진다.
    
1. 캐시 처리 가능 (Cacheable)
    - HTTP의 명세를 따르는 REST의 특징 덕분에 `캐시` 사용이 가능하다.
      (캐시의 장점으로 이전에 사용했던 데이터를 재사용할 수 있다.)
